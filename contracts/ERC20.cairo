...
func _transfer{
    ...
    # validates amount <= sender_balance and returns 1 if 1
...
func approve{
    ...
    let uint_amount = Uint256(amount,0)
    ERC20_approve(spender, uint_amount)
    return (1)
end
...
func exchange{
    ...
    let uint_amount = Uint256(amount*10**18,0)

    let (balance: Uint256) = ERC20_balances.read(caller)
    let (enough_balance) = uint256_le(uint_amount, balance)
    with_attr error_message("ERC20: burn amount exceeds balance"):
        assert enough_balance = 1
    end

    ERC20_transferFrom(caller, contract_address, uint_amount)
    return ()
end
